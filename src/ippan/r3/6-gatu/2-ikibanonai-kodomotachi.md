...
自閉症・情緒障害特別支援学級について、全体的にまとまった資料はインターネット上に見あたりません。自分なりに調べてざっくりとまとめてみました。
...
次に、今見たチャートを合わせて全体像を見てみます。なお、令和2年度の「通級に通う児童数についての都道府県別データ」がなかったため、すべて令和元年度のデータを用いてプロットしています。
...
合計した数値は約5%と、ほかの自治体と似通ったものになりましたが、東京都だけ極端に「通級に偏重している」ことが分かります。

**4-2. 中学校**

次に、中学校についても同じデータを見てみます。

設置割合も、通う生徒の割合も、数値として多少は増えますが、小学校と同様に最低位です。
...
この計画は10年以上前に立てられているわけですが、すでに次のような指摘もなされています。
...
10年以上も前に実施計画で直接指摘されているも関わらず、東京都で、通級に極端に偏重してしまったままでいる状況は、驚くばかりです。

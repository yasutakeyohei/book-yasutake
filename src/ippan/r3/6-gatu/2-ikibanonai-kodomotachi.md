...
まず、各都道府県での「自閉症・情緒障害特別支援学級の設置数」を、その都道府県での「（単式）学級の総数＋特別支援学級の総数」で割ったものを示します。つまり、その自治体（都道府県）内で、「（複式ではない）学級数の総数に対し、どれくらいの割合で自閉症・情緒障害特別支援学級が設置されているか」という割合となります。

ほかの自治体は約5%以上です。10%を超える自治体が11あることから、少なくともそれだけの設置割合が必要なのかもしれません。しかし、チャート上に、（赤い）中抜きの四角で示した東京都は、全国で最低の0.5%です。ほかの低い自治体と比べても一桁低くなっています。
...
次に、各都道府県での「自閉症・情緒障害特別支援学級に通う児童数」を、その都道府県での「（単式）学級に通う児童の総数＋特別支援学級に通う児童の総数」で割ったものを示します。つまり、その自治体（都道府県）内で、「（複式学級ではない）学級に通う児童の総数に対し、どれくらいの割合で自閉症・情緒障害特別支援学級に通う児童がいるか」という割合となります。この割合が「通級指導の対象となる潜在的な児童の存在割合」に近いほど、好ましい状況と言えると思います。

トップの岡山は4.1%で、東京都以外の自治体は、約1%以上です。それに対し、東京都は1桁ほど低い0.1%です。
...
東京都は、自閉症・情緒障害特別支援学級の設置割合も、通う児童数も、全国で最低です。なぜでしょうか。ひとつ考えられるのは「知的障害特別支援学級に肩代わりをさせているのではないか」ということです。

そこで、知的障害特別支援学級に通う児童数の割合もチャートにしてみました。しかし、こちらも東京都は低位にあります。つまり、知的障害特別支援学級が肩代わりをしているわけではありません。
...
次に考えられるのは、「通級に、自閉症・情緒障害特別支援学級の役割を担わせようとしているのではないか」ということです。

そこで、通級に通う児童数の割合もチャートにしました（令和2年度のデータがなかったので、令和元年度のものです）。
...
次に、全体の状況を見るために、各都道府県において

1. 自閉症・情緒障害特別支援学級
1. 知的障害特別支援学級
1. 通級

の合計を、その都道府県での「（単式）学級に通う児童の総数＋特別支援学級に通う児童の総数」で割った割合を、チャートに示しました。

なお、「令和2年度の通級に通う児童数について」は都道府県別データがなかったため、このチャートだけ、すべて令和元年度のデータを用いてプロットしています。
...
このチャートからは、東京都について、次のことが分かります。

- 上記1から3の特別支援については約5%の割合であり
- 通級に偏重している
...
まずは自閉症・情緒障害特別支援学級の設置割合です。小学校と同様に最低ですが、割合としては小学校の0.5%が、中学校では0.8%となります。
...
次に、自閉症・情緒障害特別支援学級に通う生徒の割合です。これも、小学校と同様に最低ですが、割合としては小学校の0.11%だったのが、中学校では0.16%となります。
...
知的障害特別支援学級に通う生徒の割合は、小学校が1.2%だったのが中学校では1.6%となります。
...
次に、通級に通う生徒の割合です。全国的に小学校と比べると低くなります。
